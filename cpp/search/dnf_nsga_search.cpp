// nsga_dnf_search.cpp
// NSGA-II style multi-objective search over DNFParams,
// calling a Python black-box evaluator.
...
#include <nlohmann/json.hpp>     // from cpp/param_data_structure/include/nlohmann/json.hpp
...
// -----------------------------------------------------------------------------
//
// NOTE: This assumes you run the executable from the project root (C:\dsa_project).
// For now we reuse the toy Python evaluator in playground/toy_search_pipeline.
// Later, you can swap PYTHON_CMD_PREFIX to point to a real wrapper around bb_run.
...
/*
    ...
    Example mapping:
      obj0 = k90_ms           (want fast response)
      obj1 = ripple           (want low ripple)
      obj2 = steps            (fewer steps ~ more efficient/stable)
...
int main() {
    ...
    for (int gen = 0; gen < N_GEN; ++gen) {
        ...
        // Logging: how many pass spec, what's the best (sum of objectives)
        int pass_count = 0;
        double best_sum = std::numeric_limits<double>::infinity();
